...
def test_p2_non_integer_takes_the_guard_on_line_41():
    with pytest.raises(RomanError) as exc:
        to_roman("MCMXCIV")
    assert "must be an integer" in str(exc.value)


def test_p2_float_takes_the_guard_on_line_41():
    with pytest.raises(RomanError) as exc:
        to_roman(4.0)
    assert "must be an integer" in str(exc.value)


# bool subclasses int, so the first operand of line 41 is false and the second decides.
def test_p2_bool_reaches_the_second_operand_of_line_41():
    with pytest.raises(RomanError) as exc:
        to_roman(True)
    assert "must be an integer" in str(exc.value)


def test_p2_false_reaches_the_second_operand_of_line_41():
    with pytest.raises(RomanError) as exc:
        to_roman(False)
    assert "must be an integer" in str(exc.value)
...
def test_p1_while_body_executes_once():
    assert to_roman(1000) == "M"


# Three iterations on the same pair exercise the redefinition of remaining.
def test_p1_while_body_executes_repeatedly():
